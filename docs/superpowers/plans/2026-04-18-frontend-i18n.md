...
- `frontend/js/app.js`:
  ...
  - `renderDashboard()`, `renderQuickAction()`, `renderToolInfo()`, `renderComingSoon()`, `renderEmptyState()`, `renderModule()`, `renderQC()`, `renderTrimming()`, `renderGffConvert()`, `renderGffConvertResult()`, `renderStarIndex()` (preserve prefill logic), `renderStarAlign()`, `renderDifferential()`, `renderNetwork()`, `renderCustomPlotPanel()`, `renderSettings()`, `renderLogPanel()` — replace in-scope literal strings with `t(...)`
...
**Edit E — Alignment & Quantification section** (lines ~140–156). The section now includes a GFF Convert item (added after plan was written). Replace:
```html
...
- [ ] **Step 5: `renderStarIndex()` and `renderStarAlign()` (line ≈591, ≈644)**

Replace `renderStarIndex()`. **Preserve the existing `prefill` / `gtfValue` logic added for the GFF Convert → STAR Index handoff** — only translate UI chrome:
...
- [ ] **Step 5b: `renderGffConvert()` (line ≈543) and `renderGffConvertResult()` (line ≈747)**

Replace `renderGffConvert()`:
...
Replace `renderGffConvertResult()`:
